...
impl SpotifyEnvParams {
  pub fn new() -> SpotifyEnvParams{
    SpotifyEnvParams {
      telegram_key: env::var("TGM_TOKEN").unwrap_or_else(|_| String::from("")),
      chat_id: env::var("TGM_CHAT_ID").unwrap_or_else(|_| String::from("")),
      giphy_key: env::var("GIPHY_TOKEN").unwrap_or_else(|_| String::from("")),
    }
  }
}

impl Default for SpotifyEnvParams {
  fn default() -> Self {
    SpotifyEnvParams::new()
  }
}
...
impl SpotifyInRussia<'_> {
  ...
  pub fn check_and_send(&self, send_cond: &str) -> Option<bool> {
    ...
    let last_news = self.mdz.get_the_last_news().unwrap_or_else(|_| String::from("В мире ничего не произошло"));
